...
* in levels of a given phenotype
    + as a `data.frame` or a stacked `r Biocpkg("ggplot2")` `geom_bar` layer
        of counts
...
evalSeparately(expr = vcfRules, envir = vcf)
```
...
\fixme{
Offer a stacked dot-plot for situations like this one, whereby density lines
display vertical lines of height 1 and rectangles of height 0.5 for phenotype
levels showing only 1 or 2 variants, respectively.
}
