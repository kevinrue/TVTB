...
* **VCF scan parameters**
    ...
    + `info` slot automatically populated with `vep(x)`
...
argument of the `r Biocpkg("VariantAnnotation")` `readVcf`. In this case,
`TVTBparam` object is used to import variants overlapping
...
* The [earlier](#ImportGRangesList) coercion of `TVTBparam` to `ScanVcfParam`
    populated the `which` slot of `svp` with "reduced" (*i.e.* non-overlapping)
    genomic ranges defined in the `ranges` slot.
* Only the INFO key defined in the `vep` slot is imported
* Only the matrix of called genotypes is imported
...
|       Class       | Motivation                                              |
| :--------------:  | :------------------------------------------------------ |
|   `FilterRules`   | Filter rules applied to a full `VCF` object (rules must |
|  (parent class)   | refer explicitely to slots of the `VCF` object).        |
|                   |                                                         |
|  `VcfFixedRules`  | Filter rules applied to the `fixed` slot of a           |
|                   | `VCF` object.                                           |
|                   |                                                         |
|   `VcfInfoRules`  | Filter rules applied to the `info` slot of a            |
|                   | `VCF` object.                                           |
|                   |                                                         |
|   `VcfVepRules`   | Filter rules applied to the Ensembl VEP predictions     |
