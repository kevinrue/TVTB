...
The Variant Tool-Box (*TVTB*[^1]) offers functions to summarise and visualise
...
[^1]: *In Italian*: ti voglio tanto bene

# Installation

The Variant Tool-Box (`r Biocpkg("TVTB")`) can be installed using the
following code:
...
\bioccomment{
Here, the 'if' statement ensures that the package is not updated if it is
already installed. To update the package, run the code ignoring the 'if'
statement.
}
...
fancy features that I have in mind
(*e.g.* plots faceted by range, showing positive and negative hits).
}
...
Functionalities in `r Biocpkg("TVTB")` currently only use
...
Currently, the major `r Biocpkg("TVTB")` functionalities include:
