...
* Stored in INFO keys of an `ExpandedVCF` objects
...
* in levels of a phenotype
    + as a `r Biocpkg("ggplot2")` `geom_density`
...
initialInfo <- colnames(info(vcf))
vcf <- addFrequencies(
    vcf = vcf, phenos = list(super_pop = c("EUR", "AFR")),
...
summary tables not yet implemented in the package.
}
...
\fixme{
Filter rules resulting in a combination of `NA` and `TRUE` throw an error
(`NA` and `FALSE` return an acceptable overall `FALSE` result).
The Bioconductor Core Team has been contacted to discuss the behaviour of
the `eval` method in the presence of `NA` and `TRUE` test results.
}
