...
* Added/overwritten in INFO keys of an `ExpandedVCF` objects
    + overall counts and frequencies (*i.e.* across all samples)
    + counts and frequencies within level(s) of phenotype(s)

**VCF filter rules**

* Filter rules on the `fixed` slot of an `ExpandedVCF` object
* Filter rules on the `info` slot of an `ExpandedVCF` object
* Filter rules on the Ensembl VEP predictions stored in a given slot
of an `ExpandedVCF` object
* The above rules can be combined and used to subset an `ExpandedVCF` object

**Counts of discrete Ensembl VEP predictions**

* in levels of a given phenotype
...
For instance, the method `addOverallFrequencies` uses the definition of
reference homozoygote (*REF*), heterozygote (*HET*),
and homozygote alternate (*ALT*) genotypes stored in a `TVTBparam` object
...
present either in the `header` or the `info` slots of the `ExpandedVCF` object.
}

## Adding frequencies in phenotype level {#FrequenciesPhenoLevel}

Similarly, the method `addPhenoLevelFrequencies` obtains the count of each genotype in samples associated with a given level of a given phenotype,
...
Notably, the `addOverallFrequencies` method is synonym to the `addFrequencies`
method called with the argument `phenos` given as a list where `names` are phenotypes, and values are `character` vectors of levels to
process within each phenotype:

```{r addFrequenciesPhenoLevel}
initialInfo <- colnames(info(vcf))
vcf <- addFrequencies(
    vcf = vcf, phenos = list(super_pop = c("EUR", "AFR")), param = tparam,
    force = TRUE)
setdiff(colnames(info(vcf)), initialInfo)
```
...
## Definition of VCF filter rules {#FilterUse}
