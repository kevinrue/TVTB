...
* Suffixes of INFO keys where counts and frequencies of genotypes are store:
...
(this occurs if they overlap multiple `GRanges`). Note that genetic
insertions and deletions may still overlap distinct non-overlapping `GRanges`
...
\bioccomment{
The vertical density lines and rectangles are artefacts produced when
...
for advice on the VCF file format and Ensembl VEP predictions.
Prof. Martin Wilkins for his trust.
