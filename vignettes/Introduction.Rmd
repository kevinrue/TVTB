...
abstract:
    The VCF Tool Box (*TVTB*) provides S4classes and methods to filter,
    ...
    those functionalities integrated in an programming-free environment.
vignette: >
...
The VCF Tool Box (`r Biocpkg("TVTB")`) offers novel S4 classes and methods
...
* in levels of a phenotype
    ...
    + as a `data.table` listing all values faceted by phenotype level
...
\bioccomment{
INFO key suffixes should be considered carefully to avoid overwriting
data in INFO keys already present in the `VCF` object.
}
...
`r Biocpkg("VariantAnnotation") `readVcf` method, and
`ExpandedVCF` objects result of the
`r Biocpkg("VariantAnnotation") `expand` method applied to
a `CollapsedVCF` object.

`VCF` objects may include any information imported from VCF file(s)
that users deem relevant for the analysis;
...
* `info(x)`
    + No minimal requirement.
    + **Recommended**: `<vep>`: where `<vep>` stands for a user-defined
    INFO key where
...
\fixme{
In the future, recommend `geno(x)[["GQ"]]`, if available.
This could enable consideration of individual genotypes on the basis
of passing a genotype quality cut-off; for instance when counting genotypes.
}

## Genomic ranges {#ImportGRanges}

Currently, the `ranges` slot of `TVTBparam` objects is not used by any
method in the package.
In the future, this slot may be used to produce summary statistics
and plots faceted by range (e.g. burden test; comparison of genomic regions).
Genomic ranges could represent gene-coding regions, exons, promoters,
*etc*.

\bioccomment{
In the meantime, users may store genomic ranges of interest in this slot,
thereby reducing the number of objects in their workspace.
For instance, the `VariantAnnotation` `ScanVcfParam` class accepts
`GenomicRanges` `GRanges` objects (in the `which` slot) that describes the
sequences and ranges to be queried from VCF file(s).
}

\fixme{
The `ranges` slot of the `TVTBparam` class may actually be re-designed to
contain `GRangesList`.
Summary statistics and plots could be produced for each element of the
`GRangesList`, while each element could be composed of multiple genomic ranges.
For instance, burden test considering only the variants in the sets of exons
corresponding to a list of genes.
}
...
Those data may be extracted, replaced, subsetted to refine plots and
...
point to avoid calling multiple times the same time-consuming methods.
}
...
for each genomic feature affected by variants
