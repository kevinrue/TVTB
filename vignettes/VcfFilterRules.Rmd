...
abstract: >
    ...
    by indices or names of variant records.
    The *TVTB* package extends `FilterRules`
...
(*i.e.* size and number of objects in the workspace).
...
* *VCF filter rules* can be toggled individually between an active and
    an inactive states
* *VCF filter rules* can be subsetted, replaced, and deleted
...
* A rule named `"avgSuperPopAF"` calculates the average of the allele frequencies
    calculated in each the five super-populations
...
for all variants,
which leaves a final `2` variants passing the remaining active filter rules:
