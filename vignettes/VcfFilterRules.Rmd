...
abstract: >
    `VCF` objects of the `VariantAnnotation` package may be subsetted
    by indices or variant (row)names. The *TVTB* package extends `FilterRules`
    of the `S4Vectors` package to provide filter rules applicable to
    `VCF` objects. These *VCF filter rules* facilitate filtering of variants
    using combinations of filters applicable to FIXED fields, INFO fields,
    and Ensembl VEP predictions stored in a given INFO field.
vignette: >
...
provides a powerful tool to define flexible 
and lightweight filter rules in the form of
`expression` and `function` objects, evaluated within a given environment.
The `r Biocpkg("TVTB")` package extends this existing `FilterRules` class
into novel classes of *VCF filter rules*,
each applicable to information stored in the distinct slots of `VCF`
...
|                   | stored in a given INFO key of an `VCF` object.  |
|                   |                                                         |
...
* individual *VCF filter rules* can be toggled between active and inactive
    states
* *VCF filter rules* can be subsetted and replaced

To account for the structure of `VCF` objects, some of the new
*VCF filter rules* classes implemented in the `r Biocpkg("TVTB")` package
require additional information stored in new slots of the 
filter objects, associated with new accessor methods.
For instance:

* `VcfVepRules` require the INFO key where predictions of the
    Ensembl Variant Effect Predictor are stored in a `VCF` object.
    The `vep` accessor method may be used to query this slot.
* `VcfFilterRules`---which may combine any number of filter rules stored in
    `VcfFixedRules`, `VcfInfoRules`, `VcfVepRules`, and other
...
as demonstrated in a [later section](#AltField).

# Fields available for the definition of filter rules {#VcfFields}

Let us first examine which fields (*i.e.* columns)
...
# Filter rules using a single raw field {#SingleFieldFilter}

The value of a particular field can be used to define an `expression`
that represents a simple filter rule based on that value.
Multiple rules may be stored in any one `FilterRules` objects.
In addition, filter rules should ideally be named
to facilitate their use, subsetting, and (de-)activation,
...
* A rule named `pass` identifies variants for which the value in the `FILTER` 
    field is `"PASS"`
* A rule named `qual20` identifies variants where the value in the `QUAL` field
...
\bioccomment{
Deactivated rules (*e.g.* `qual20`) return `TRUE` for all variants.
}

# Filter rules using multiple raw fields {#MultipleFieldsFilter}

It is also possible for a `FilterRules` to use multiple fields
...
are one of the four nucleotides (*i.e.* single nucleotide polymorphisms; SNPs):
...
\fixme{
The `append` method (check also the `c` method)
currently returns a `FilterRules`
object when applying it to any of the child classes.
The `append` method must be overridden for each child class defined in `TVTB`.
For instance, the following code should return a `VcfFixedRules` object:
    fixedRules <- append(fixedRules, VcfFixedRules(list(
        SNP = expression(
        as.character(REF) %in% nucleotides &
            as.character(ALT) %in% nucleotides)
    )))
...
# Filter rules involving calculations {#CalculationFilter}

The `expression` that defines filter rules may also include calculations.
In the chunk of code below, two simple filter rules are defined
...
* A rule named `avgSuperPopAF` calculates the average of the allele frequencies
    calculated in each the five super-populations, and
...
# Filter rules as functions {#FunctionFilter}
...
* defines a filter rule using the above function
...
while being evaluated using the `VCF` object as environment.
}
...
# Pattern matching {#PatternFilter}
...
* the `exact` filter rule matches only the given value associated
    with `27` variants,
* the `grepl` filter rule also matches an extra two variants
    associated with the value `"missense_variant&splice_region_variant"`
    (but not the `"missense_variant"` value)

# Using ALT data in `CollapsedVCF` and `ExpandedVCF` objects {#AltField}
...
## `ExpandedVCF` {#ExpandedVCF}

A key aspect of `ExpandedVCF` objects is that the `ALT` field of their
`info` slot may only store a single allele per record as a `DNAStringSet`
object.

In an earlier section that demonstrated
[Filter rules using multiple raw fields](#MultipleFieldsFilter),
`ALT` data of a `ExpandedVCF` object had to be re-typed from `DNAStringSet`
to `character` before the `%in%` function could be applied.

Nevertheless, *VCF filter rules* may also make use of methods associated with
the `DNAStringSet` class.
For instance, insertions may be identified using the fields `REF` and `ALT`
of the `fixed` slot:
...
Here, the above `VcfFixedRules` is synonym to an independent
...
## `CollapsedVCF` {#CollapsedVCF}

In contrast to `ExpandedVCF`, `CollapsedVCF` may contain more than one allele
per record in their `ALT` field, represented by a `DNAStringSetList` object.
...
of *VCF filter rules*, the resulting `VcfFilterRules` object can be evaluated
and used to subset `VCF` objects exactly like any of the classes defining
more specific *VCF filter rules*.

During the creation of `VcfFixedRules` objects, each *VCF filter rule*
being combined is marked with a *type* value,
indicating the VCF slot that the filter rule must be applied to.
...
Clearly, the *VCF filter rules* `SNP` and `isInsertion` are mutually exclusive,
which explains the final `0` variants left after filtering.
...
As a result, the deactivated filter rule now returns `TRUE` for all variants,
which leaves a final `2` variants passing the remain active filter rules:

* INFO/FILTER equal to `"PASS"`
* INFO/QUAL greater than or equal to `20`
...
Notably, when `VcfFilterRules` objects are being combined,
the `type` and `active` value of each filter rule is transferred in
the combined object.
...
It is now possible to combine the two `VcfFilterRules`, even with another
`VcfInfoRules` object:
