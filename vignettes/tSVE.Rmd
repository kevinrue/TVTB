...
* First-time users are encouraged to follow this vignette
    ...
    + This vignetted was designed to be read beside an open *R* session with
...
* The `r Biocpkg("ensembldb")` package and relevant `EnsDb`[^2] annotation
    package are required if that interface is used to query genomic ranges.
...
Certain sections of this vignette ([example](#InputEnsDb))
require that the
...
* The web-application automatically trims `","` characters from the text input.
...
variants located on a single chromosome.
...
This panel allows users to select a pre-installed `EnsDb` annotation package.
...
each variant contributes at most one prediction count to each feature.
...
Ensembl VEP predictions associated with variants observed in the different
levels of a given phenotype, and to display the distribution of
those predictions in the distinct phenotype levels.
...
> * Select `super_pop` as the *Phenotype field*
> * Select `Density_&_Dot_plot` as the *Layer(s)*
> * Select `Feature` as the *VEP faceting key*
