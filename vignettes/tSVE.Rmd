...
* The `r Biocpkg("EnsDb.Hsapiens.v75")` is currently required to query
    genomic ranges associated by gene name(s)[^3].
...
one or more VCF file(s) annotated by the
[Ensembl Variant Effect Predictor (VEP)](http://www.ensembl.org/info/docs/tools/vep)
script [@RN1].
...
each variant contributes at most one prediction count to each facet.
